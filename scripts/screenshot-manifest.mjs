...
Tracking → Day Log inside the Tracking window (no nested Agenda / Activity Log). Shared palette includes **Log activity**. Day nav; Planned Xm estimated · Task logs Xm. Painted occupancy strip. One hour grid with more room per hour: solid = tracked (display-depth color; ≈ if assumed); dashed = planned (click to confirm it happened — paint, optional notes; tasks completeTask and unlock dependents); amber = task timeLogs. Sleep and long blocks are sliced so every occupied hour is clickable. Click a painted block → Entry dialog. Day notes under every Tracking view.`,
  ...
  },
  {
    ...
    description: `Same Lists chrome (two-bay toolbar + VIEW mode deck). List view: Search lists…; named rows with type icons — click/dblclick opens. Select-mode checkboxes when Select is on.`,
  },
  {
    ...
    description: `Same Lists chrome (VIEW mode deck). Folder Details table: columns for name / type / counts — dblclick opens. (List content Details is separate: no complete/missed ticks.)`,
  },
  {
    ...
    description: `Same Lists chrome (VIEW mode deck). Cards: title, description, counts; + add, settings, delete; Eye opens list. Smart to-do cards show due empty copy.`,
  },
  {
    ...
    description: `Open-list chrome: actions bay (Up / New / Import / Settings / Select + Search) + DISPLAY mode deck (LED keys: Default / Checklist / Icons / Details / Spreadsheet). Address. Sidebar. Inner caption = Default. Reading rows (pip + name; no complete ticks). Right inspector: Add / Bulk add / List Settings / Change Icon / Pin — Delete only in List Settings. Status + Smart lists.`,
  },
  {
    ...
    description: `Same open-list chrome + DISPLAY mode deck. Checklist: Completed ticks (optional Missed); Select overlays selection ticks. Click name → detail.`,
  },
  {
  ...
  // ── Scheduler ─────────────────────────────────────────────────────────────
  {
    ...
    description: `Scheduler — Funnel. Views: Funnel / Gantt / Dependencies. Periods: Always / Year / Month / Week / Day. Available Tasks + Filters & Sort. Task rows drag onto two columns of cards: This Year, This Month, Next Month, This Week, Next Week, Today (real local date), Tomorrow (real local date), Eventually / Later (Next Actions list eventually, no period). Empty cards say Empty. An unfinished past period rolls up one level (day→week→month→year→unscheduled) unless it was pushed; prior placements stay visible in gray past cells.`,
  },
  {
