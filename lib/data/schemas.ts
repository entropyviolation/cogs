...
export const linkStanceSchema = z.enum([
  "strong-support",
  "weak-support",
  "none",
  "weak-refute",
...
// Unlike most schemas here this one is strict, not `.passthrough()`, so a field
// missing from it is *dropped* rather than merely unvalidated — `stance` and
// `weight` carry the belief graph (`lib/belief-strength.ts`) and were being
// stripped on restore until they were listed.
export const itemLinkSchema = z.object({
  id: z.string(),
  relation: z.string(),
  targetId: z.string(),
  stance: linkStanceSchema.optional(),
...
export const subtaskSchema = z
  .object({
    ...
    isMolecular: z.boolean().optional(),
...
export const taskSchema = z
  .object({
    ...
    missedAt: dateLike.optional(),
    startedAt: dateLike.optional(),
    status: z.enum(["active", "done", "partial", "deferred", "cancelled", "missed"]).optional(),
    // Provenance for autogenerated values (lib/estimated-values.ts). Lenient so
    // new estimate kinds/fields round-trip without rejecting historical records.
    estimates: z
      .array(
        z
          .object({
