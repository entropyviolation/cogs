...
| `module-templates.ts` | Pre-built workspace **module templates** (Itinerary, Cleaning, Budget, Book Tasting, Film DNA Lab, Blank): `buildModuleTemplate` (pure — lists + attribute schemas + seed items + views) and `instantiateModuleTemplate` (commits to stores). Itinerary v2 seeds Plan (`doc`), Itinerary (`itinerary-doc`), Activities (`trip-map`), plus City Places; Film DNA Lab seeds the `film-dna` view + Films list. Unit-tested. | §8 |
