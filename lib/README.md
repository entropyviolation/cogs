...
| `smart-parse.ts` | Pure, LLM-free smart-capture parser: leading colon paths (`list: item`, `folder: list: item`), `parsePathHeader` for Bulk Add, plus dates/times/priority/duration/`cat:` hints → `SmartSuggestion` + highlights. `now` injectable. | §4, §10 |
| `capture-target.ts` | Resolve/create folders+lists from a capture path and build the task (Inbox vs skip-Inbox / All Items). Used by Quick Add and Bulk Add. | §4 |
