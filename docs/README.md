...
| `CANONICAL_FIELDS.md` | Read-only analysis snapshot of the COGS data model: which fields are canonical, legacy/duplicate, or derived. Reference for a future, careful field-cleanup pass. |
...
| `screenshots/` | Full-page PNG captures of every major app view plus matching `.txt` write-ups. Re-capture with `npm run capture-screenshots` while `npm run dev` is running. See [`screenshots/README.md`](screenshots/README.md). |
