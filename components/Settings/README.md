...
| File | Purpose | Spec |
|------|---------|------|
| `SettingsDialog.tsx` | The app's **Settings** entry point — a header-launched dialog (`SettingsDialog`) that hosts **Home location** (`<HomeLocationField />` — city for Plan sunrise/sunset, default San Diego), full backup/restore (renders `<BackupRestore />`), **Mobile Sync** (`<MobileSyncPanel />` — manual hub push/pull), and a one-click **"Set up Second Brain"** action that seeds the Source + Belief item types via `useItemTypeStore.seedSecondBrainTypes`. Wired into the global header in `app/page.tsx`. | §3.2 |
