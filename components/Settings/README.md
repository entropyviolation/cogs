...
| File | Purpose | Spec |
|------|---------|------|
| `SettingsDialog.tsx` | The app's **Settings** entry point — a header-launched dialog (`SettingsDialog`) that hosts cross-cutting utilities: full backup/restore (renders `<BackupRestore />`), **Mobile Sync** (`<MobileSyncPanel />` — live sync is parked; manual hub push/pull only), and a one-click **"Set up Second Brain"** action that seeds the Source + Belief item types via `useItemTypeStore.seedSecondBrainTypes`. Wired into the global header in `app/page.tsx`. | §3.2 |
