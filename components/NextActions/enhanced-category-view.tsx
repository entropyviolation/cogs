...
// Add ConvertToOperationDialog (reuse the same implementation as in task-detail-popup)
      ...
      <div className="flex h-full">
        ...
        <Sidebar className="border-r bg-muted/30">
          ...
          <SidebarContent>
            <SidebarMenu>
              <SidebarMenuItem>
                <SidebarMenuButton isActive={!selectedFolderId} onClick={() => setSelectedFolderId(null)}>
                  Home
                </SidebarMenuButton>
              </SidebarMenuItem>
              {folders.map((folder) => (
        ...
        <div className="flex-1 p-6 space-y-6">
                              ...
                              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                {!task.isOperation && (
                                  <Button
                                    variant="ghost"
                                    size="icon"
                                    className="h-7 w-7 text-purple-600 hover:text-purple-700 focus-ring"
                                    onClick={(e) => {
                                      e.stopPropagation()
                                      setConvertTask(task)
                                    }}
          ...
          {/* Convert to Operation Dialog for category view */}
          {convertTask && (
            <ConvertToOperationDialog
              open={!!convertTask}
              onClose={() => setConvertTask(null)}
              onConvert={(title, goals) => {
                ...
                // Update the task
                updateTask({ ...convertTask, isOperation: true, operationId: opId, categories: [...(convertTask.categories || []), categoryId] })
                setConvertTask(null)
              }}
            />
          )}
