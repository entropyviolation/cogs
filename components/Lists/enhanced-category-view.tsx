...
export function EnhancedCategoryView({ onTaskSelect }: EnhancedCategoryViewProps) {
  ...
  const removeCategoryFromFolder = useTaskStore((s) => s.removeCategoryFromFolder)
  ...
  const dedupeCategories = useTaskStore((s) => s.dedupeCategories)
  ...
  const drag = useListsDragDrop({ folders, updateTask, addCategoryToFolder, removeCategoryFromFolder })
  const selection = useListsSelection()
  const taskActions = useListsTaskActions(allTasks, categories, folders, addTask, updateTask)
  ...
  const getTasksForCategory = useCallback(
    (categoryId: string) => allTasks.filter((t) => t.categories?.includes(categoryId) && !t.completed),
    [allTasks],
  )
  ...
  const getCategoryCompletionRate = useCallback(
    (categoryId: string) => {
      const categoryTasks = allTasks.filter((t) => t.categories?.includes(categoryId))
  ...
  const entries = useMemo(
    ...
    [isHome, isAll, currentFolder, folders, categories, homePinned, showSmartLists, allTasks, getSmartTasks, getTasksForCategory, countForFolder],
  )
  ...
  const openIconKey = openCategory ? iconFor(openCategory.id, openCategory.icon) : isRootAll ? iconFor("lists-root", undefined) : openFolderAll && currentFolder ? iconFor(currentFolder.id, currentFolder.icon) : openSmart ? orbFor(openSmart.id) : openHabits ? orbFor("daily-habits") : orbFor("lists-root")
  ...
  const handleCreateCategory = useCallback(() => {
    ...
    if (currentFolder) addCategoryToFolder(currentFolder.id, id)
  ...
  }, [newCategoryName, newCategoryColor, newCategoryDescription, newCategoryScheduleable, newCategoryTemplate, addCategory, categories.length, currentFolder, addCategoryToFolder, isHome, toggleHomePin])
  ...
  const performCsvImport = useCallback(() => {
    ...
    if (!categoryId) {
      ...
      addCategory({ id: categoryId, name: listName || "Imported list", color: newCategoryColor, description: `Imported from ${csvImport.fileName}`, createdAt: new Date(), order: categories.length, scheduleable: true, itemAttributes: attrDefs })
    ...
    setCsvImport(null)
  }, [csvImport, addCategory, addTask, updateCategory, categories, newCategoryColor, currentFolder, addCategoryToFolder, isHome, toggleHomePin, taskActions])
    ...
    if (folderView === "icons") {
      return (
        <FolderViewIcons
          ...
          handleIconCanvasDrop={(e) => drag.handleIconCanvasDrop(e, isAll, currentFolder)}
  ...
  return (
    <div className="fm98" style={{ height: "calc(100vh - 150px)", minHeight: 560 }}>
      <div className="fm-window">
        ...
        <div className="fm-window-body">
          <ListsToolbar
            ...
            onAutoOrganize={() => autoOrganizeIcons(location, entries.map((e) => `${e.kind}-${e.id}`))}
          />
          ...
          {selectMode && (
            <div className="fm-toolbar" style={{ marginTop: 3 }}>
              ...
              {folders.map((folder) => (
                <button key={folder.id} className="fm-btn fm-btn-sm" disabled={selectedCategories.length === 0} onClick={() => { selectedCategories.forEach((catId) => addCategoryToFolder(folder.id, catId)); selection.cancelSelectMode() }}>
                  → {folder.name}
                </button>
              ))}
            </div>
            ...
            {openCategory && (
              <div className="fm-sidebar" style={{ width: 150 }}>
                <div style={{ padding: 6, display: "flex", flexDirection: "column", gap: 6 }}>
                  ...
                  <button className="fm-btn fm-btn-sm fm-btn-danger" onClick={() => { if (confirm(`Delete list "${openCategory.name}"?`)) { deleteCategory(openCategory.id); closeTarget() } }}>Delete List</button>
                </div>
              </div>
            )}
      ...
      <EditListDialog editingCategory={editingCategory} onEditingCategoryChange={setEditingCategory} folders={folders} homePinned={homePinned} listDisplay={listDisplay} setListDisplay={setListDisplay} toggleHomePin={toggleHomePin} onOpenIconPicker={() => editingCategory && setIconPickerFor({ kind: "category", id: editingCategory.id })} onSave={handleEditCategory} onDelete={() => { if (editingCategory && confirm(`Delete list "${editingCategory.name}"?`)) { deleteCategory(editingCategory.id); if (openTarget?.type === "category" && openTarget.id === editingCategory.id) closeTarget(); setEditingCategory(null) } }} />
