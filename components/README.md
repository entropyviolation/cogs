...
| Scheduler | `Scheduler/` | Win95 window around the period funnel (Always → Year → Month → Week → Day) + Gantt / Dependencies documents |
...
| `inbox.tsx` / `inbox.css` | Inbox dialog (`data-ui-name="Inbox"`; nested clarify is `"Clarify idea"`): **milled fascia** (brushed silver bay, engraved uppercase nameplates, raised metal keys, CRT count wells `#7dffc4`, round power lamp on the active partition). **Inbox** (revisit) and **Monkey brain** (dump) as equal bay keys. Open ideas in the current pile, **newest first**. Walk **selected** ideas (`TaskClarificationDialog` — rename, discard, recent lists), +1 / +50 when the revisit Inbox hits 0, Select all / Deselect all, multi-select → list (**Apply** stays in Inbox, **Apply and clarify** files onto those lists) / deadline / merge / **Mark clarified** / move between piles / **Bulk edit** / **delete** (Are you sure?). Navy selection outline. Looks only — same verbs and accessible names. |
