...
| Modules | `Modules/` | User-built mini-app **workspaces** (Itinerary / House Cleaning / Budget / Book Tasting / Film DNA templates + custom views) and dashboard widgets |
...
| `Modules/` | Module platform — full-screen workspaces (`workspace/`, incl. itinerary, filmrecs, housecleaning views) + dashboard widgets |
...
| `ItemTypes/` | Create/edit user-defined item types (Settings → Manage Item Types) |
...
| `Settings/` | `SettingsDialog` header entry — `BackupRestore` (full app JSON backup/restore), item types, Second Brain setup, and manual mobile hub pull |
...
`docs/SPEC_MAPPING.md` §3). Plan text
...
- Item detail lives under `components/ItemDetail/` (`ItemDetailPage` + `ItemDetailPopup`).
